...
type RaceVisualConfig = {
  type: VehicleType;
  className: string;
  color1: string;
  color2: string;
  speedLinesClassName: string;
...
type SideVehicleVisualProps = RaceVisualConfig & {
  isHoverable: boolean;
};

const SideVehicleVisual = ({
  type,
  className,
  color1,
  color2,
  ...
  flipped = false,
  isHoverable,
}: SideVehicleVisualProps) => {
  const VehicleComponent = type === 'car' ? Car : Motorcycle;

  return (
    <div className={`relative${flipped ? ' transform scale-x-[-1]' : ''}`}>
      <VehicleComponent className={className} color1={color1} color2={color2} isHoverable={isHoverable} />
      <SpeedLines className={speedLinesClassName} />
      <motion.div
        className={`absolute inset-0 ${glowClassName} rounded-full blur-xl ${
          isHoverable ? 'opacity-0 group-hover:opacity-100' : 'opacity-40'
        } -z-10`}
...
const RacingVehicle = ({ direction, onComplete, visual, startX, startY, travelX }: RacingVehicleProps) => (
  <motion.div
  ...
  >
    <SideVehicleVisual {...visual} isHoverable={false} flipped={direction === 'right-to-left'} />
  </motion.div>
);
...
export default function VehicleDecorations() {
  ...
  const launchRace = (visual: RaceVisualConfig, direction: RaceDirection, event: MouseEvent<HTMLButtonElement>) => {
    ...
    const travelX =
      direction === 'left-to-right'
        ? window.innerWidth - startX + 300
        : -startX - 300;
    setRaces((prev) => [...prev, { id, type: visual.type, direction, visual, startX, startY, travelX }]);
  };
    ...
    <>
        ...
        <motion.button
          ...
          onClick={(event) =>
            launchRace(
              {
                type: 'car',
                className: 'w-28 md:w-36 drop-shadow-lg transition-all duration-300',
                color1: '#ec4899',
                color2: '#f97316',
                speedLinesClassName: 'w-12 text-pink-400/60 absolute -left-8 top-1/2 -translate-y-1/2',
                glowClassName: 'bg-pink-500/30',
              },
              'left-to-right',
              event
            )
        ...
        <motion.button
          ...
          onClick={(event) =>
            launchRace(
              {
                type: 'motorcycle',
                className: 'w-24 md:w-32 drop-shadow-lg transition-all duration-300',
                color1: '#22d3ee',
                color2: '#3b82f6',
                speedLinesClassName: 'w-10 text-cyan-400/60 absolute -left-6 top-1/2 -translate-y-1/2',
                glowClassName: 'bg-cyan-500/30',
              },
              'left-to-right',
              event
            )
        ...
        <motion.button
          ...
          onClick={(event) =>
            launchRace(
              {
                type: 'car',
                className: 'w-32 md:w-40 drop-shadow-lg transition-all duration-300',
                color1: '#a855f7',
                color2: '#6366f1',
                speedLinesClassName: 'w-14 text-purple-400/60 absolute -left-10 top-1/2 -translate-y-1/2',
                glowClassName: 'bg-purple-500/30',
              },
              'left-to-right',
              event
            )
      ...
      <div className="absolute right-0 top-0 bottom-0 w-32 md:w-48 overflow-hidden z-30">
        ...
        <motion.button
          type="button"
          aria-label="Race motorcycle across the screen"
          className="absolute top-32 -right-2 cursor-pointer group bg-transparent border-0 p-0 z-40"
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
            ...
            launchRace(
              {
                ...
                color1: '#f97316',
                color2: '#eab308',
                speedLinesClassName: 'w-10 text-orange-400/60 absolute -left-6 top-1/2 -translate-y-1/2',
        ...
        <motion.button
          type="button"
          aria-label="Race car across the screen"
          className="absolute top-1/2 -translate-y-1/2 -right-4 cursor-pointer group bg-transparent border-0 p-0 z-40"
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
            ...
            launchRace(
              {
                ...
                color1: '#10b981',
                color2: '#22d3ee',
                speedLinesClassName: 'w-12 text-emerald-400/60 absolute -left-8 top-1/2 -translate-y-1/2',
                glowClassName: 'bg-emerald-500/30',
              },
              'right-to-left',
              event
            )
        ...
        <motion.button
          type="button"
          aria-label="Race motorcycle across the screen"
          className="absolute bottom-32 -right-0 cursor-pointer group bg-transparent border-0 p-0 z-40"
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
